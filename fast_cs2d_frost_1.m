...
if (strcmp(trans_mode,'BDCT') | strcmp(trans_mode,'BWHT')|strcmp(trans_mode,'OSTM'))& (nargin<5)
   if rand_type==0,
       blk_size=32;
   else
       blk_size=m;
...
% User can change it
%Perm_state = 3587642;
Perm_state = 3000000+round((rand+1)/2*999999);
...
% Main loop to test the reconstruction performance for each K(i);
 for i =1:length(K),
       ...
       % Dense FFT-based Operators;
        ...
        elseif strcmp(trans_mode,'BDCT') | strcmp(trans_mode,'BWHT')|strcmp(trans_mode,'OSTM')
           ...
           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           AA=sign(randn(1,blk_size/2+1));
%A=ones(1,513);
Bx=zeros(1,blk_size);
Bx(1:blk_size/2+1)=AA(1:blk_size/2+1);
for count=1:(blk_size/2-1);
    Bx((blk_size/2+1)+count)=AA((blk_size/2+1)-count);
end
...
A=toeplitz(aa,aa)/blk_size;
           %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            Phi = @(z) blk_f1d_frost_1(z, select_vect, rand_vect, trans_mode, blk_size,A);
            % Define the transpose of the Sampling Operator;
            Phi_T = @(z) blk_t1d_frost_1(z, N, select_vect, rand_vect,trans_mode, blk_size,A); 
